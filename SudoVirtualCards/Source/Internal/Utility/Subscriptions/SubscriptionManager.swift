...
actor DefaultSubscriptionManager: SubscriptionManager {
    ...
    /// Utility for decrypting transaction details.
    ...
    init(graphQLClient: SudoApiClient, unsealer: Unsealer) {
    ...
    func subscribe(id: String, notificationType: SubscriptionNotificationType, subscriber: Subscriber, owner: String) {
        ...
        case .transactionUpdated:
            subscribe(
                subscription: GraphQL.OnTransactionUpdateSubscription(owner: owner),
                notificationType: notificationType
            )
        case .transactionDeleted:
            subscribe(
                subscription: GraphQL.OnTransactionDeleteSubscription(owner: owner),
                notificationType: notificationType
            )
        }
    }
    ...
    func getNotification<D: GraphQLSubscriptionResult>(
        ...
        case .transactionUpdated:
            guard
                let updateData = data as? GraphQL.OnTransactionUpdateSubscription.Data,
                let graphQLTransaction = updateData.onTransactionUpdate,
                let unsealedTransaction = try? unsealer.unseal(graphQLTransaction.fragments.sealedTransaction)
            else {
                return nil
            }
            return .transactionUpdated(unsealedTransaction)

        case .transactionDeleted:
            guard
                let deleteData = data as? GraphQL.OnTransactionDeleteSubscription.Data,
                let graphQLTransaction = deleteData.onTransactionDelete,
                let unsealedTransaction = try? unsealer.unseal(graphQLTransaction.fragments.sealedTransaction)
            else {
                return nil
            }
