...
public class DefaultSudoVirtualCardsClient: SudoVirtualCardsClient {
    ...
    internal init(
        ...
        self.virtualCardService = VirtualCardService(graphQLClient: graphQLClient, unsealer: unsealer, logger: logger)
    ...
    public func updateVirtualCard(withInput input: UpdateVirtualCardInput)async throws -> VirtualCard {
        try checkUserSignedIn()
        let keyPairResult = publicKeyService.getCurrentKeyPair(generateKeyIfNotFound: true)
        let keyPair: KeyPair
    ...
    public func cancelVirtualCard(withId id: String) async throws -> VirtualCard {
        try checkUserSignedIn()
        let keyPair: KeyPair
        do {
            let keyPairResult = publicKeyService.getCurrentKeyPair(generateKeyIfNotFound: true)
    ...
    public func listProvisionalCards(
        ...
        typealias UnsealedCardPairs = (provisionalCard: ListProvisionalCardsQuery.Data.ListProvisionalCard.Item, unsealedCard: VirtualCard?)
        let unsealedCardPairs: [UnsealedCardPairs] = try listCards.items.map({
            var unsealedCard: VirtualCard?
    ...
    func getPublicKeyWithId(_ id: String, cachePolicy: CachePolicy) async throws ->PublicKey? {
        try checkUserSignedIn()
        let query = GetPublicKeyForVirtualCardsQuery(keyId: id, keyFormats: nil)
        let data = try await GraphQLHelper.performQuery(graphQLClient: graphQLClient, query: query, cachePolicy: cachePolicy, logger: logger)
    ...
    func getPublicKeys(
        ...
        try checkUserSignedIn()
        let query = GetPublicKeysForVirtualCardsQuery(limit: limit, nextToken: nextToken, keyFormats: nil)
        let data = try await GraphQLHelper.performQuery(graphQLClient: graphQLClient, query: query, cachePolicy: cachePolicy, logger: logger)
    ...
    func getPublicKeys(
        ...
        try checkUserSignedIn()
        let query = GetKeyRingForVirtualCardsQuery(keyRingId: keyRingId, limit: limit, nextToken: nextToken, keyFormats: nil)
        let data = try await GraphQLHelper.performQuery(graphQLClient: graphQLClient, query: query, cachePolicy: cachePolicy, logger: logger)
